...
with open('synset_words.txt') as f:
	synsets = pandas.DataFrame([
		{'synset_id': l.strip().split(' ')[0], 'name': ' '.join(l.strip().split(' ')[1:]).split(',')[0]} for l in f.readlines() ])

synsets_wnid = synsets['synset_id'].values
synsets_name = synsets['name'].values
...
for i in range(0,len(matlab_layers)):
  if matlab_layers[i]['type'] == 'conv':
    matlab_layers[i]['biases'] = numpy.ascontiguousarray(matlab_layers[i]['biases'])
    matlab_layers[i]['filters'] = numpy.ascontiguousarray(matlab_layers[i]['filters'].transpose([3,2,1,0]))
...
mkn['averageImage'] = mkn['averageImage'].transpose([2,0,1])
mkn['averageImage'] = mkn['averageImage'][None,::-1,:,:].astype(np.float32)
mkn['averageImage'] = numpy.ascontiguousarray(mkn['averageImage'])

mnet = {
  'layers': matlab_layers,
  'normalization': mkn['averageImage'],
  'classes': synsets_name}

pickle.dump(mnet, open(args.output.name,'wb'))
